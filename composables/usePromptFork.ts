...
/**
 * Ossuary Fork Workflow — generate editable prompt templates from artwork schemas
 *
...
const categoryMeta: Record<TechniqueCategory, { label: string; icon: string }> = {
  ...
  post: { label: 'Post-Processing', icon: '✨' },
}
...
export function usePromptFork() {
  ...
  function generateTemplate(options: ForkOptions): string {
    ...
    lines.push(`// Generated by Ossuary — lapinski.art`)
    lines.push('')
  ...
  /**
   * Generate a minimal "quick fork" — just the prompt with technique tags as comments
   */
  ...
  /**
   * Fork an artwork — generate template and copy to clipboard
   */
  ...
  /**
   * Quick fork — copy just the raw prompt with technique tags
   */
