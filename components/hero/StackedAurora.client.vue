...
const vertexShader = `
  ...
  void main() {
    ...
    // Mouse influence — subtle offset to noise coordinates
    float mouseInfluence = snoise(vec3(pos.x * 0.3 + uMouse.x * 0.5, pos.y * 0.3 + uMouse.y * 0.5, uTime * 0.15)) * 0.3;

    // Multi-layered noise displacement
    float noise1 = snoise(vec3(pos.x * 0.4, pos.y * 0.4, uTime * 0.12 + uScrollProgress * 2.0)) * uWaveAmplitude;
    float noise2 = snoise(vec3(pos.x * 0.8 + 5.0, pos.y * 0.8, uTime * 0.08 - uScrollProgress)) * uWaveAmplitude * 0.4;
    float noise3 = snoise(vec3(pos.x * 1.5 + 10.0, pos.y * 1.5, uTime * 0.2)) * uWaveAmplitude * 0.15;
...
const fragmentShader = `
  ...
  void main() {
    // Base colors from the site palette
    ...
    // Create gradient based on displacement + UV
    float gradientY = vUv.y;
    float gradientX = vUv.x;

    // Mix colors based on displacement and position
    vec3 baseColor = mix(dark900, dark800, gradientY * 0.7 + vDisplacement * 0.3);

    // Add lavender glow in displaced areas
    float glowIntensity = smoothstep(-0.2, 0.8, vDisplacement) * 0.35;
    vec3 glowColor = mix(lavender400, lavender300, gradientX + sin(uTime * 0.3) * 0.3);
    baseColor = mix(baseColor, glowColor, glowIntensity);

    // Accent red spark on peaks — very subtle
    float peakIntensity = smoothstep(0.6, 1.2, vDisplacement) * 0.15;
    baseColor = mix(baseColor, accentRed, peakIntensity);

    // Scroll-driven color shift: warmer as user scrolls deeper
    float warmth = uScrollProgress * 0.12;
    baseColor += vec3(warmth * 0.3, warmth * 0.05, -warmth * 0.1);

    // Edge fade to dark-900 so it blends with the page background
